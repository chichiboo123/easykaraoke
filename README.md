...
반주를 듣고 Space 키로 한국어 음절 타이밍을 입력해, 노래방 영상을 브라우저에서 만드는 local-first 정적 웹앱입니다. 선택 사항인 뮤지컬 모드를 켜면 배역을 구분할 수 있습니다. 음원·가사·배경·글꼴·프로젝트와 렌더 결과는 서버로 전송하지 않습니다.

## 핵심 기능

- MP3/WAV/M4A/AAC 재생, 클릭·드래그 앤 드롭, 파형, seek와 볼륨
- 일반 가사 입력과 선택형 뮤지컬 모드(`[배역] 가사`, 배역별 색상), `Intl.Segmenter` 기반 한국어 음절 분리
- Space 타이밍 입력, 입력 지연 보정, 0.5×/0.75× 재생, 취소/미세 이동/숫자 수정/음절 합치기와 분리, A–B 반복
- 동일 Canvas 렌더러를 쓰는 16:9 미리보기와 출력, 연속적인 음절 fill
- 6개 디자인, 내장/사용자 글꼴, 사용자 배경, IndexedDB 자동 저장
- 브라우저가 제공하는 MP4 또는 WebM MediaRecorder를 이용한 1080p 30fps 실시간 스트리밍 출력
...
최신 데스크톱 Chrome/Edge처럼 `canvas.captureStream`과 `MediaRecorder`를 제공하는 환경을 권장합니다. MP4를 지원하지 않는 브라우저에서는 WebM으로 내보내며, 둘 다 지원하지 않는 경우 내보내기 화면에서 미리 안내합니다. 출력은 메모리에 모든 프레임을 쌓지 않고 실시간으로 녹화하므로 곡 길이만큼 시간이 걸리며, 내보내는 동안 탭을 화면에 유지해야 합니다. 정밀 편집은 PC를 권장합니다.

## 폰트

기본값은 실제 브라우저에 항상 존재하는 시스템 글꼴입니다. `public/fonts/TJJoyofsingingM.otf` 등 선택 글꼴은 **배포 권한을 확인한 원본**을 배치해야 활성화됩니다. 파일이 없으면 해당 카드를 비활성화하고 사용자에게 시스템 대체 상태를 표시합니다. 자세한 내용은 [FONT_LICENSE_NOTES.md](FONT_LICENSE_NOTES.md)를 확인하세요.
