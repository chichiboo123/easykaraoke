...
async function openRecent(id) { project = (await loadProjects()).find(x => x.id === id); audioBlob = await loadFile(id, 'audio'); bgBlob = await loadFile(id, 'background'); fontBlob = await loadFile(id, 'font'); if (audioBlob)
...
function lyricsStep() { return `<h1>2. 가사와 배역</h1><p><code>[배역] 가사</code> 형식으로 붙여넣으면 배역도 한 번에 만들어져요.</p><textarea id="lyrics" rows="7" placeholder="[왜] 왜 저럴까 정말 그럴까\n[전체] 우리 함께 문을 열어!">${esc(project.lyricBlocks.map(b => `[${project.roles.find(r => r.id === b.roleId)?.name || '전체'}] ${b.text}`).join('\n'))}</textarea><button id="apply" class="primary">가사 카드 만들기</button><div class="role-list"><h2>배역 색상</h2>${project.roles.map(r => `<label class="role"><input type="color" data-rolecolor="${r.id}" value="${r.color}"><input data-rolename="${r.id}" value="${esc(r.name)}" aria-label="배역 이름"></label>`).join('')}</div><div class="cards">${project.lyricBlocks.map((b, i) => `<article><span>${i + 1}</span><select data-blockrole="${b.id}">${project.roles.map(r => `<option value="${r.id}" ${r.id === b.roleId ? 'selected' : ''}>${esc(r.name)}</option>`).join('')}</select><input data-blocktext="${b.id}" value="${esc(b.text)}"><button data-up="${b.id}" aria-label="위로">↑</button><button data-down="${b.id}" aria-label="아래로">↓</button></article>`).join('')}</div>`; }
function bindLyrics() { $('#apply').onclick = () => change(() => { const r = parseLyrics($('#lyrics').value, project.roles); project.roles = r.roles; project.lyricBlocks = r.blocks; stampIndex = 0; }); document.querySelectorAll('[data-rolecolor]').forEach(x => x.onchange = () => change(() => project.roles.find(r => r.id === x.dataset.rolecolor).color = x.value)); document.querySelectorAll('[data-rolename]').forEach(x => x.onchange = () => change(() => project.roles.find(r => r.id === x.dataset.rolename).name = x.value)); document.querySelectorAll('[data-blockrole]').forEach(x => x.onchange = () => change(() => project.lyricBlocks.find(b => b.id === x.dataset.blockrole).roleId = x.value)); document.querySelectorAll('[data-blocktext]').forEach(x => x.onchange = () => change(() => { const b = project.lyricBlocks.find(b => b.id === x.dataset.blocktext); b.text = x.value; b.segments = segmentKorean(x.value).map(t => ({ id: uid(), text: t, start: 0, end: 0 })); })); for (const dir of ['up', 'down'])
