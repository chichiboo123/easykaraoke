...
/**
 * 파형 · 블록 · 재생헤드를 Canvas 하나로 그린다.
 * V1은 음절 1개당 DOM 버튼 1개라, 500음절 곡이면 Space 한 번에 500개 노드를 다시 만들었다.
 * 여기서는 노드가 0개다. 스탬프는 캔버스를 한 번 더 그릴 뿐이다.
 */

export type TimelineHit =
  | { kind: 'seek'; time: number }
  | { kind: 'block'; index: number; edge: 'start' | 'end' | 'move'; time: number };
...
export type ViewWindow = { from: number; to: number };

/** 지금 확대 배율에서 화면에 담기는 시간 길이. */
export function spanFor(p: KaraokeProject, widthPx: number): number {
  const duration = Math.max(1, p.media.duration);
  return Math.min(duration, widthPx / (100 * p.view.zoom));
}

/**
 * 보여 줄 시간 구간.
 * follow가 켜져 있으면 재생헤드를 따라가고, 사용자가 직접 스크롤하면
 * 그 위치를 지킨다. 예전에는 늘 재생 위치에 붙어 있어 앞뒤를 볼 수 없었다.
 */
export function windowFor(p: KaraokeProject, time: number, widthPx: number): ViewWindow {
  const duration = Math.max(1, p.media.duration);
  const span = spanFor(p, widthPx);
  const maxFrom = Math.max(0, duration - span);
  let from = p.view.follow ? time - span * 0.4 : p.view.from;
  from = Math.max(0, Math.min(maxFrom, from));
  return { from, to: from + span };
}

/** 창을 특정 시작 시각으로 옮긴다. 범위를 벗어나지 않게 잘라 준다. */
export function clampFrom(p: KaraokeProject, from: number, widthPx: number): number {
  const duration = Math.max(1, p.media.duration);
  return Math.max(0, Math.min(Math.max(0, duration - spanFor(p, widthPx)), from));
}

export function draw(canvas: HTMLCanvasElement, p: KaraokeProject, time: number, win: ViewWindow, selected: number, dark: boolean): void {
  ...
  const xOf = (t: number) => ((t - win.from) / span) * cssW;
  ...
  p.blocks.forEach((b, i) => {
    if (b.end <= b.start || b.end < win.from || b.start > win.to) return;
    const x = xOf(b.start);
...
export function hitTest(canvas: HTMLCanvasElement, p: KaraokeProject, win: ViewWindow, clientX: number, clientY: number): TimelineHit {
  const rect = canvas.getBoundingClientRect();
  const x = clientX - rect.left;
  const y = clientY - rect.top;
  const span = win.to - win.from || 1;
  const time = win.from + (x / rect.width) * span;
  const xOf = (t: number) => ((t - win.from) / span) * rect.width;
  ...
  if (y >= WAVE_H + 6 && y <= WAVE_H + BLOCK_H) {
    for (let i = 0; i < p.blocks.length; i++) {
      ...
      if (Math.abs(x - bx) <= EDGE_PX) return { kind: 'block', index: i, edge: 'start', time };
      if (Math.abs(x - (bx + bw)) <= EDGE_PX) return { kind: 'block', index: i, edge: 'end', time };
