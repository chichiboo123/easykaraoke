...
export function finishScreen(rerender: () => void, go: (step: 1 | 2 | 3) => void): HTMLElement {
  const p = project();
  ...
  const sceneBox = el('div', { class: 'stack' });
  const thumbs: HTMLButtonElement[] = [];
  for (const group of SCENE_GROUPS) {
    const grid = el('div', { class: 'thumb-grid' });
    for (const scene of SCENES.filter((x) => x.group === group)) {
      const thumb = el('canvas', { class: 'thumb-canvas', width: 384, height: 216 });
      const b = el('button', {
        class: `thumb${p.style.preset === scene.id ? ' is-chosen' : ''}`,
        type: 'button',
        title: scene.note,
      }, [thumb, el('b', { textContent: scene.label })]);
      b.onclick = () => {
        commit('preset', () => {
          project().style.preset = scene.id;
        });
        for (const other of thumbs) other.classList.remove('is-chosen');
      ...
      queueMicrotask(() => {
        ...
        ctx.setTransform(384 / 1920, 0, 0, 216 / 1080, 0, 0);
        renderFrame(
          ctx,
          { ...project(), style: { ...project().style, preset: scene.id }, intro: { countdown: false, seconds: 0 } },
          previewTime,
          state.bgPrepared,
        );
      });
    }
    sceneBox.append(el('div', { class: 'group-label', textContent: group }), grid);
  }
  if (state.files.background) {
    sceneBox.prepend(el('p', { class: 'hint', textContent: '배경 이미지를 넣으면 무대 대신 그 이미지가 쓰여요. 배경 탭에서 지울 수 있어요.' }));
  }
    ...
    fontGrid.replaceChildren(
      ...fonts.map((f) => {
        const b = el('button', {
          ...
          type: 'button',
          disabled: !f.ready,
          title: f.ready ? f.label : f.source === 'cdn' ? '인터넷에서 이 글꼴을 불러오지 못했어요' : '이 글꼴 파일이 없어서 쓸 수 없어요',
        });
        if (f.ready) b.style.fontFamily = `"${f.family}", system-ui, sans-serif`;
        b.append(el('b', { textContent: f.label }), el('span', { textContent: '우리 함께 노래해요!' }));
        if (!f.ready) b.append(el('em', { class: 'badge', textContent: f.source === 'cdn' ? '못 불러옴' : '파일 없음' }));
        b.onclick = async () => {
          commit('font', () => {
          ...
          buildFonts();
          redraw();
          // 고른 글꼴로 가사를 그리려면 해당 글자 조각이 먼저 와 있어야 한다.
          await ensureGlyphs(f.family, textForProject(project()));
          redraw();
        };
        return b;
      }),
  ...
  const fontPick = filePicker({
    ...
    onPick: async (file) => {
      try {
        ...
        buildFonts();
        redraw();
        await ensureGlyphs(family, textForProject(project()));
        redraw();
  ...
  if (p.musicalMode) {
    ...
    sel.append(
      el('option', { value: 'common', textContent: '모두 같은 색', selected: p.style.colorMode === 'common' }),
      el('option', { value: 'role', textContent: '배역마다 다른 색', selected: p.style.colorMode === 'role' }),
    );
    sel.onchange = () => {
      commit('colorMode', () => {
        project().style.colorMode = sel.value as 'common' | 'role';
      });
      redraw();
    };
  ...
  // 한 화면에 다 쌓으면 끝없이 스크롤해야 한다. 캡컷처럼 탭으로 나눠 한 번에 하나만 보여준다.
  const tabs: [string, string, HTMLElement][] = [
    ['무대', 'wallpaper', sceneBox],
    ['글자', 'text_fields', el('div', { class: 'stack' }, [fontGrid, fontPick])],
    [
      '배경',
      'image',
      el('div', { class: 'stack' }, [
    ...
    ['저장', 'movie', exportPanel(go)],
  ];

  const tabBar = el('div', { class: 'tab-bar', role: 'tablist' });
  const tabBody = el('div', { class: 'tab-body' });
  tabs.forEach(([label, ic, body], i) => {
    const t = button({ kind: 'ghost', icon: ic, label, onClick: () => select(i) });
    t.classList.add('tab');
    t.setAttribute('role', 'tab');
    tabBar.append(t);
    body.hidden = i !== activeTab;
    tabBody.append(body);
  });
  ...
  queueMicrotask(() => {
    ...
    redraw();
    ensureGlyphs(project().style.fontFamily, textForProject(project())).then(redraw);
    // 시작할 때 못 받은 CDN 글꼴이 그새 도착했을 수 있다.
    refreshCdnFonts().then((changed) => {
      if (changed) buildFonts();
    });
    stopFinish();
    const loop = () => {
  ...
  function slider(label: string, key: 'brightness' | 'darken' | 'blur', min: number, max: number, step: number, after: () => void): HTMLElement {
...
function exportPanel(go: (s: 1 | 2 | 3) => void): HTMLElement {
  const p = project();
  ...
  const run = async (range?: { from: number; to: number }, height: 1080 | 720 = 1080) => {
...
function check(ok: boolean, title: string, detail: string): HTMLElement {
