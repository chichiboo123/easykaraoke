...
const DEMO: { text: string; start: number; end: number }[] = [
...
function outlined(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, w: number, fill: string) {
  ctx.lineJoin = 'round';
  ctx.lineWidth = w;
...
function disc(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, spin: number, alpha: number) {
  ctx.save();
...
function nextSongBar(ctx: CanvasRenderingContext2D) {
...
function titlePanel(ctx: CanvasRenderingContext2D, appear: number) {
  const x = 430;
  const y = 402;
  const w = 900;
  const h = 250;
...
function credits(ctx: CanvasRenderingContext2D, appear: number) {
  ctx.save();
  ctx.globalAlpha = appear;
  ctx.textAlign = 'right';
  ctx.textBaseline = 'middle';
...
function countdown(ctx: CanvasRenderingContext2D, remain: number) {
...
function demoLyric(ctx: CanvasRenderingContext2D, t: number) {
...
function progressBar(ctx: CanvasRenderingContext2D, t: number) {
...
export function drawHero(ctx: CanvasRenderingContext2D, elapsed: number): void {
  ...
  if (t >= CARD_OUT && t < CUE) countdown(ctx, CUE - t);
  if (t >= CUE) demoLyric(ctx, t);
