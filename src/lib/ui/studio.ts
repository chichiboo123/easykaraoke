...
let raf = 0;
let win: ViewWindow = { from: 0, to: 10 };
 ...
 */
export function studioScreen(rerender: () => void, go: (step: 1 | 2 | 3) => void): HTMLElement {
  const p = project();
  ...
  const syncRates = () => {
    for (const b of Array.from(rates.children) as HTMLButtonElement[]) b.classList.toggle('is-on', Number(b.dataset.rate) === project().timing.rate);
  };
  ...
  const tuning = el('div', { class: 'tuning' }, [
    ...
    button({
      ...
      onClick: () => {
        ...
        if (i < 0) return toast('모든 줄을 다 찍었어요!', 'success');
        select(i);
  ...
  modeToggle.onchange = () => {
    ...
    touch(() => {
      ...
      if (modeToggle.checked) {
        ...
        // 이미 찍은 줄이면 끝까지 찍은 상태로, 새 줄이면 처음부터.
        fineIndex = block && block.end > block.start ? block.segments.length : 0;
      } else {
    ...
    el('div', { class: 'cue-actions' }, [
      ...
      button({
        ...
        onClick: () => {
          ...
          if (i < 0) return toast('취소할 것이 없어요.');
          commit('unstamp', () => {
  ...
  function togglePlay() {
    resume().then(() => {
      if (audio.paused) audio.play().catch(() => toast('재생할 수 없어요. 반주 파일을 다시 넣어 주세요.', 'error'));
      else audio.pause();
      syncPlayIcon();
    });
  }
  ...
  function select(index: number) {
    ...
    if (cur.timing.mode === 'syllable' && canFine(cur.blocks[index])) {
      ...
      cur.timing.fineBlock = block.id;
      fineIndex = block.end > block.start ? block.segments.length : 0;
      renderFine();
    }
    renderCards();
  }
   ...
   */
  function stamp(eventTime?: number) {
    const cur = project();
  ...
  const canFine = (b?: { kind?: string; segments: unknown[] }) => !!b && b.kind !== 'interlude' && b.segments.length > 0;

  function fineBlockIndex(): number {
    const cur = project();
    const byId = cur.blocks.findIndex((b) => b.id === cur.timing.fineBlock);
    if (byId >= 0 && canFine(cur.blocks[byId])) return byId;
    if (canFine(cur.blocks[state.selected])) return state.selected;
    // 아직 아무것도 안 찍었으면 타이밍이 없는 첫 줄부터 시작한다.
    const untimed = cur.blocks.findIndex((b) => canFine(b) && b.end <= b.start);
    if (untimed >= 0) return untimed;
    return cur.blocks.findIndex((b) => canFine(b));
  }

  function nextFineIndex(from: number): number {
    const cur = project();
    for (let i = from + 1; i < cur.blocks.length; i++) if (canFine(cur.blocks[i])) return i;
    return -1;
  }

  /**
   * 아직 안 찍은 글자들을 줄 끝에 모아 둔다.
   * 이렇게 해 두면 찍는 도중에도 미리보기가 이상하게 보이지 않는다.
   */
  function settleTail(block: ReturnType<typeof project>['blocks'][number], from: number) {
    const segs = block.segments;
    if (from > 0 && from <= segs.length) segs[from - 1].end = block.end;
    for (let i = from; i < segs.length; i++) {
      segs[i].start = block.end;
      segs[i].end = block.end;
    }
  }

  /**
   * 글자 단위로 찍기. 곡 전체를 끊김 없이 이어서 찍는다.
   *
   * 한 줄의 글자를 다 찍으면 그 탭이 줄을 닫고 다음 줄을 바로 시작한다.
   * 그래서 줄을 일일이 골라 줄 필요가 없다.
   * 색이 차는 기준은 여전히 줄이고, 그 안에서 글자마다 속도가 정해진다.
   */
  function fineStamp(eventTime?: number) {
    ...
    const HOLD = 0.3; // 아직 안 찍은 부분의 잠정 길이

    commit('fine-stamp', () => {
      let block = cur.blocks[bi];

      const beginLine = (index: number, at: number) => {
        bi = index;
        block = cur.blocks[bi];
        cur.timing.fineBlock = block.id;
        state.selected = bi;
      ...
      if (fineIndex <= 0) {
        // 줄의 첫 글자 — 여기서 줄이 시작된다.
        const prev = cur.blocks.slice(0, bi).reverse().find((b) => canFine(b) && b.end > b.start);
        if (prev && prev.end > now) prev.end = Math.max(prev.start + 0.1, now);
        beginLine(bi, now);
      } else if (fineIndex < block.segments.length) {
        // 줄 안에서 글자 경계를 하나 확정한다.
        block.segments[fineIndex - 1].end = now;
        block.segments[fineIndex].start = now;
        block.end = Math.max(now + HOLD, block.end);
        fineIndex++;
        settleTail(block, fineIndex);
      } else {
        // 마지막 글자까지 찍었다 — 이 탭이 줄을 닫고 다음 줄을 연다.
        block.segments[block.segments.length - 1].end = Math.max(block.segments[block.segments.length - 1].start + 0.05, now);
        block.end = block.segments[block.segments.length - 1].end;
        const next = nextFineIndex(bi);
        if (next < 0) {
          fineIndex = block.segments.length;
          return;
        }
        beginLine(next, now);
      }
      cur.timing.cursor = Math.max(cur.timing.cursor, bi + (fineIndex >= cur.blocks[bi].segments.length ? 1 : 0));
    });

    if (fineIndex >= cur.blocks[bi].segments.length && nextFineIndex(bi) < 0) {
      toast('마지막 줄까지 다 찍었어요!', 'success');
    }
    refreshAll();
  }

  /** 고른 글자의 경계를 조금씩 민다. 다시 찍지 않고 손보고 싶을 때. */
  function nudgeBoundary(deltaMs: number) {
    const cur = project();
    const bi = fineBlockIndex();
    const block = cur.blocks[bi];
    if (!block) return;
    const index = Math.min(Math.max(1, fineIndex), block.segments.length - 1);
    if (index < 1) return;
    commit(`fine-nudge-${block.id}`, () => {
      setSyllableStart(block, index, block.segments[index].start + deltaMs / 1000);
    });
    renderFine();
  }

  /** 지금 찍는 줄의 글자들을 띠로 보여 준다. 어디까지 왔는지 한눈에. */
  function renderFine() {
    const cur = project();
    ...
    const block = cur.blocks[bi];
    if (!block) {
      fineStrip.replaceChildren(el('p', { class: 'hint', textContent: '가사를 먼저 넣어 주세요.' }));
      return;
    }
    const total = block.segments.length;
    const done = Math.min(fineIndex, total);
    const timed = block.end > block.start;

    const head = el('div', { class: 'fine-head' }, [
      el('small', {}, [
        el('b', { textContent: `${bi + 1}번 줄` }),
        el('span', { textContent: ` · 글자 ${done}/${total}` }),
        el('span', { class: 'muted', textContent: timed ? ` · ${clock(block.start)} – ${clock(block.end)}` : ' · 아직 안 찍음' }),
      ]),
      el('div', { class: 'fine-tools' }, [
        button({ kind: 'ghost', icon: 'chevron_left', title: '이 경계 10ms 앞으로', fallback: '−10', onClick: () => nudgeBoundary(-10) }),
        button({ kind: 'ghost', icon: 'chevron_right', title: '이 경계 10ms 뒤로', fallback: '+10', onClick: () => nudgeBoundary(10) }),
        button({
          kind: 'ghost',
          icon: 'restart_alt',
          label: '이 줄 다시',
          title: '이 줄을 처음부터 다시 찍어요',
          onClick: () => {
            fineIndex = 0;
            seek(Math.max(0, block.start - 1));
            renderFine();
            toast(`${bi + 1}번 줄을 처음부터 다시 찍어요.`);
          },
        }),
        button({
          kind: 'ghost',
          ...
          label: '균등하게',
          title: '글자 간격을 고르게 되돌려요',
          onClick: () => {
            if (!timed) return toast('먼저 이 줄을 찍어 주세요.');
    ...
    block.segments.forEach((seg, i) => {
      ...
      const chip = el(
        'button',
        {
          ...
          type: 'button',
          title: timed ? `${clock(seg.start)} – ${clock(seg.end)}` : '아직 안 찍음',
        },
        [el('b', { textContent: seg.text }), el('small', { textContent: len > 0.005 ? `${len.toFixed(2)}` : '–' })],
      );
      chip.onclick = () => {
        // 이 글자부터 이어서 찍는다. 앞 글자 시작으로 돌아가 준비할 시간을 준다.
        fineIndex = i;
        const at = i > 0 ? block.segments[i - 1].start : block.start;
        if (timed) seek(Math.max(0, at - 0.6));
        renderFine();
      };
      chips.append(chip);
    });

    const tip = el('p', { class: 'fine-tip' }, [
      el('span', { textContent: fineIndex <= 0 ? '재생하고 이 줄이 시작되는 순간 Space를 누르세요.' : '글자가 넘어가는 순간마다 Space. 마지막 글자 다음 Space는 다음 줄로 넘어갑니다.' }),
    ]);

    fineStrip.replaceChildren(head, chips, tip);
  }
  ...
  function blockCard(b: ReturnType<typeof project>['blocks'][number], i: number, cursor: number): HTMLElement {
    ...
    if (project().musicalMode && role) card.style.setProperty('--role', role.color);
    ...
    // 카드 가운데는 대개 가사 입력란이다. 거기를 눌러도 그 줄이 선택되어야
    // "카드를 누르면 그 자리로 이동"이라는 안내와 어긋나지 않는다.
    text.onfocus = () => select(i);
    text.onchange = () => {
    ...
    if (project().musicalMode) {
      ...
      for (const r of project().roles) sel.append(el('option', { value: r.id, textContent: r.name, selected: r.id === b.roleId }));
      sel.onchange = () => {
        commit(`role-${b.id}`, () => {
          b.roleId = sel.value;
        });
  ...
  timeline.addEventListener('pointerdown', (e) => {
    ...
    if (hit.edge === 'move') timeline.style.cursor = 'grabbing';
  });
  ...
  timeline.addEventListener('pointermove', (e) => {
    ...
    commit(`drag-${b.id}`, () => {
      if (drag!.edge === 'start') b.start = Math.max(0, Math.min(b.end - MIN_BLOCK, hit.time));
      else if (drag!.edge === 'end') b.end = Math.max(b.start + MIN_BLOCK, Math.min(duration, hit.time));
      else {
  ...
  const tick = () => {
    ...
    if (cur.view.loopOn && cur.view.loopB > cur.view.loopA && t >= cur.view.loopB) audio.currentTime = cur.view.loopA;
...
export function nudgeSelected(deltaMs: number): boolean {
  const cur = project();
  const b = cur.blocks[state.selected];
  if (!b || b.end <= b.start) return false;
