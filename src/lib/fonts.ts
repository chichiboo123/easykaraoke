export type FontEntry = { family: string; label: string; url?: string; ready: boolean; custom?: boolean };
...
const SYSTEM: FontEntry = { family: 'system-ui', label: '기본 글꼴', ready: true };

export async function loadBuiltins(): Promise<FontEntry[]> {
  await Promise.all(
    fonts.map(async (f) => {
      if (!f.url) return;
      try {
  ...
  if (!fonts.some((f) => f.ready)) fonts.push(SYSTEM);
  else if (!fonts.includes(SYSTEM)) fonts.push(SYSTEM);
...
export async function loadCustomFont(file: File): Promise<string> {
  ...
  else fonts.push({ family, label: family, ready: true, custom: true });
  return family;
}
