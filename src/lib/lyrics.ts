 ...
 */
export function setSyllableStart(block: LyricBlock, index: number, time: number, minLen = 0.06): void {
  const segs = block.segments;
...
export function normalizeSegments(block: LyricBlock): void {
  const segs = block.segments;
  if (!segs.length) return;
  segs[0].start = block.start;
  segs[segs.length - 1].end = block.end;
  for (let i = 1; i < segs.length; i++) {
    if (segs[i].start < segs[i - 1].start) segs[i].start = segs[i - 1].start;
