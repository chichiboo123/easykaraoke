...
export function segmentProgress(s: Segment, time: number): number {
  if (s.end <= s.start) return time >= s.start && s.start > 0 ? 1 : 0;
 ...
 * V2는 그럴 때 첫 블록을 보여준다.
 */
export function blockAt(p: KaraokeProject, time: number): { block?: LyricBlock; next?: LyricBlock; index: number } {
  const blocks = p.blocks;
  if (!blocks.length) return { index: -1 };
...
export function firstCue(p: KaraokeProject): number {
