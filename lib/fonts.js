...
function withTimeout(work, ms) {
    return Promise.race([work, new Promise((ok) => setTimeout(() => ok(null), ms))]);
}
/**
 ...
 */
function stylesheetsReady(ms) {
    const links = [...document.querySelectorAll('link[rel="stylesheet"]')].filter((l) => l.href.startsWith('http'));
    const waits = links.map((l) => new Promise((ok) => {
        if (l.sheet)
            return ok();
...
export async function refreshCdnFonts() {
    let changed = false;
...
export async function loadBuiltins(timeoutMs = 6000) {
    await stylesheetsReady(Math.min(4000, timeoutMs));
    await withTimeout(Promise.all(fonts.map(async (f) => {
        if (f.source === 'system')
            return;
        try {
...
export async function ensureGlyphs(family, text) {
    const entry = fonts.find((f) => f.family === family);
    if (!entry || entry.source === 'system' || !entry.ready)
        return;
...
export async function loadCustomFont(file) {
    ...
    if (existing)
        ...
        fonts.splice(fonts.length - 1, 0, { family, label: family, source: 'custom', ready: true });
    return family;
}
