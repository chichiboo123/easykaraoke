 ...
 */
function readBoxes(v, start, end) {
    const out = [];
    let i = start;
    while (i + 8 <= end) {
 ...
 */
export async function finalizeMp4(blob) {
    try {
        ...
        for (const track of tracks.values()) {
            ...
            if (tkhd) {
                ...
                if (ver === 0)
                    ov.setUint32(tkhd.start + tkhd.hdr + 20, inMovie);
                else
                    ov.setBigUint64(tkhd.start + tkhd.hdr + 28, BigInt(inMovie));
            }
            if (mdhd) {
                const ver = ov.getUint8(mdhd.start + mdhd.hdr);
                if (ver === 0)
                    ov.setUint32(mdhd.start + mdhd.hdr + 16, track.duration);
                else
                    ov.setBigUint64(mdhd.start + mdhd.hdr + 24, BigInt(track.duration));
            }
        }
        if (!movieDuration)
            return blob;
        if (mvhdVer === 0)
            ov.setUint32(mvhd.start + mvhd.hdr + 16, movieDuration);
        else
            ov.setBigUint64(mvhd.start + mvhd.hdr + 24, BigInt(movieDuration));
        // ── mfra: 조각마다 (시각 → 파일 오프셋) ──────────
