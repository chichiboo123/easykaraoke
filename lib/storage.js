    ...
    try {
        ...
        if (kinds.length) {
            ...
            for (const kind of kinds) {
                ...
                if (blob)
                    store.put(blob, `${project.id}:${kind}`);
                else
                    store.delete(`${project.id}:${kind}`);
            }
        }
        await done(tx);
    }
...
export async function loadProjects() {
    ...
    try {
        ...
        await done(tx);
        return req.result.map(migrate).sort((a, b) => b.updatedAt - a.updatedAt);
    }
    finally {
        db.close();
    }
}
export async function loadFile(id, kind) {
    const db = await open();
    try {
...
export async function removeProject(id) {
    ...
    try {
        ...
        for (const kind of ['audio', 'background', 'font'])
            files.delete(`${id}:${kind}`);
        await done(tx);
    }
    finally {
        db.close();
