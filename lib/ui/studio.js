...
export function studioScreen(rerender, go) {
    ...
    modeToggle.onchange = () => {
        ...
        touch(() => {
            ...
            if (modeToggle.checked) {
                ...
                if (nothingTimed)
                    state.selected = cur.blocks.findIndex((b) => canFine(b));
                else if (!canFine(cur.blocks[state.selected]))
                    state.selected = cur.blocks.findIndex((b) => canFine(b) && b.end <= b.start);
                if (state.selected < 0)
                    state.selected = cur.blocks.findIndex((b) => canFine(b));
    ...
    let fineIndex = 0;
    const canFine = (b) => !!b && b.kind !== 'interlude' && b.segments.length > 0;
    function fineBlockIndex() {
        const cur = project();
        const byId = cur.blocks.findIndex((b) => b.id === cur.timing.fineBlock);
        if (byId >= 0 && canFine(cur.blocks[byId]))
            return byId;
        if (canFine(cur.blocks[state.selected]))
    ...
    function nextFineIndex(from) {
        const cur = project();
        for (let i = from + 1; i < cur.blocks.length; i++)
            if (canFine(cur.blocks[i]))
                return i;
        return -1;
    }
    /**
     ...
     */
    function settleTail(block, from) {
        const segs = block.segments;
        if (from > 0 && from <= segs.length)
            segs[from - 1].end = block.end;
        ...
        commit('fine-stamp', () => {
            ...
            if (fineIndex <= 0) {
                ...
                if (prev && prev.end > now)
                    prev.end = Math.max(prev.start + 0.1, now);
                beginLine(bi, now);
            }
...
export let studioDelete = null;
export let studioInterlude = null;
