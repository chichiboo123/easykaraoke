...
export function studioScreen(rerender, go) {
    ...
    el('div', { class: 'studio-top' }, [el('div', { class: 'stage' }, [preview]), transport]), el('div', { class: 'timeline-wrap' }, [timeline]), tuning, cuePanel, el('div', { class: 'block-head' }, [
        el('h2', { textContent: '가사 블록' }),
        el('span', { class: 'hint', textContent: '카드를 누르면 그 자리로 이동해요. 타임라인에서 블록 끝을 끌면 길이가 바뀝니다.' }),
    ]), cards, el('div', { class: 'screen-foot' }, [
    ...
    function renderCue() {
        ...
        cueQueue.replaceChildren(...cur.blocks.slice(i + 1, i + 4).map((b) => el('span', { textContent: b.text })));
