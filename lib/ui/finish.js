...
export function finishScreen(rerender, go) {
    ...
    for (const group of SCENE_GROUPS) {
        ...
        for (const scene of SCENES.filter((x) => x.group === group)) {
            ...
            queueMicrotask(() => {
                ...
                renderFrame(ctx, { ...project(), style: { ...project().style, preset: scene.id }, intro: { countdown: false, seconds: 0 } }, previewTime, state.bgPrepared);
            });
        }
    ...
    const buildFonts = () => {
        fontGrid.replaceChildren(...fonts.map((f) => {
            ...
            if (!f.ready)
                b.append(el('em', { class: 'badge', textContent: f.source === 'cdn' ? '못 불러옴' : '파일 없음' }));
            b.onclick = async () => {
                commit('font', () => {
    ...
    stagePane.append(el('div', { class: 'transport transport-slim' }, [
        playBtn,
        clockOut,
        button({
            kind: 'ghost',
