...
const isAudio = (f) => AUDIO_OK.test(f.name) || /^audio\//.test(f.type);
export async function acceptAudio(file, rerender) {
    if (!isAudio(file)) {
    ...
    try {
        ...
        commit('audio', () => {
            ...
            if (!p.view.loopB)
                p.view.loopB = Math.min(20, duration);
        });
...
export function prepareScreen(rerender, go) {
    const p = project();
    ...
    if (p.musicalMode && p.roles.length)
        lyricsBox.append(roleEditor(rerender));
    root.append(section('2', '가사 넣기', lyricsBox));
    // ── 다음 ─────────────────────────────────────────────
    const ready = hasAudio && p.blocks.length > 0;
    root.append(el('div', { class: 'screen-foot' }, [
...
function blockSummary() {
    const p = project();
    if (!p.blocks.length)
        return el('span', { class: 'hint', textContent: '아직 만든 블록이 없어요.' });
    const timed = timedCount(p.blocks);
    return el('span', { class: 'hint' }, [
        el('b', { textContent: `${p.blocks.length}줄` }),
        el('span', { textContent: timed ? ` · 타이밍 ${timed}줄 완료` : ' · 타이밍은 다음 단계에서' }),
    ]);
}
function roleEditor(rerender) {
    const p = project();
    ...
    for (const role of p.roles) {
        ...
        color.oninput = () => commit(`role-color-${role.id}`, () => {
