    ...
    try {
        ...
        await new Promise((ok, fail) => {
            ...
            stopTicker = createTicker(1000 / 30, () => {
                ...
                if (audio.ended || t >= to + 1.2 || (t >= to && to >= project.media.duration - 0.05))
                    finish();
            });
        });
        rec.stop();
        ...
        if (!chunks.length || frames < 2)
            throw new Error('녹화된 영상이 비어 있어요. 다시 시도해 주세요.');
        // 프레임이 크게 모자라면 소리만 나오는 영상이 된다. 조용히 넘기지 않는다.
        const expected = span * 30;
        if (frames < expected * 0.5) {
            throw new Error(`영상 프레임이 충분히 담기지 않았어요 (${frames}장 / 예상 ${Math.round(expected)}장). ` +
