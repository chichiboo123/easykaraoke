 ...
 */
export function segmentKorean(text) {
    const out = [];
    let prefixStart = -1;
    let cursor = 0;
...
export function parseLyrics(input, roles, musicalMode = false) {
 ...
 */
export function mergeBlocks(previous, incoming) {
    const pool = new Map();
    for (const b of previous) {
        if (b.end <= b.start || !isLyric(b))
            continue;
...
function weight(text) {
    ...
    if (code >= 0xac00 && code <= 0xd7a3)
        return (code - 0xac00) % 28 === 0 ? 1 : 1.18;
    return 1;
}
/**
 ...
 */
export function distribute(block) {
...
export function retext(block, text) {
    block.text = text;
    block.segments = segmentKorean(text);
    distribute(block);
}
/** 간주는 사용자가 찍는 대상이 아니다. 진행률과 커서 계산에서 제외한다. */
export const isLyric = (b) => b.kind !== 'interlude';
export const lyricCount = (blocks) => blocks.filter(isLyric).length;
export const timedCount = (blocks) => blocks.filter((b) => isLyric(b) && b.end > b.start).length;
/** 간주 블록. 가사가 없는 구간을 "간주중"으로 채운다. */
export function makeInterlude(start, end, roleId) {
...
/**
 * 글자 단위 편집에서 쓰는, 한 음절의 시작 시각을 직접 정하는 함수.
 *
 * 줄 전체의 start~end는 그대로 두고 그 안에서만 경계를 옮긴다.
 * 앞뒤 음절이 서로 밀리지 않게 최소 길이를 지키며 잘라 붙인다.
 */
export function setSyllableStart(block, index, time, minLen = 0.06) {
    const segs = block.segments;
    if (index <= 0 || index >= segs.length)
        return;
    const lower = segs[index - 1].start + minLen;
    const upper = segs[index].end - minLen;
    const t = Math.max(lower, Math.min(upper, time));
    segs[index - 1].end = t;
    segs[index].start = t;
}
/** 음절 경계가 어긋나지 않게 줄 구간 안으로 정리한다. */
export function normalizeSegments(block) {
    const segs = block.segments;
    if (!segs.length)
        return;
    segs[0].start = block.start;
    segs[segs.length - 1].end = block.end;
