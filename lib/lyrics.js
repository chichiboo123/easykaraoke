...
export function parseLyrics(input, roles) { const next = [...roles]; const blocks = input.split(/\r?\n/).map(x => x.trim()).filter(Boolean).map(line => { const m = line.match(/^\[([^\]]+)]\s*(.*)$/); const roleName = m?.[1]?.trim() || '전체'; const text = (m?.[2] ?? line).trim(); let role = next.find(x => x.name === roleName); if (!role) {
    role = { id: uid(), name: roleName, color: ['#60a5fa', '#c084fc', '#34d399', '#fb923c'][next.length % 4] };
    next.push(role);
} return { id: uid(), text, roleId: role.id, segments: segmentKorean(text).map(t => ({ id: uid(), text: t, start: 0, end: 0 })) }; }); return { blocks, roles: next }; }
