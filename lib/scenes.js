...
const scenes = [
    ...
    /* ── 감성 ── */
    {
        ...
        draw(ctx, t, W, H) {
            ...
            ctx.globalCompositeOperation = 'screen';
            const bands = [['#2fd6a0', 0], ['#4bb8ff', 1], ['#a86ff0', 2], ['#38e0c8', 3]];
...
export const SCENE_GROUPS = ['기본', '감성', '무대'];
...
export function drawScrim(ctx, scene, W, H) {
    if (!scene.scrim)
        return;
...
export function finishScene(ctx, scene, W, H) {
    if (scene.vignette > 0) {
